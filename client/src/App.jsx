...
function ensureExpense(row, fallbackCurrency = "USD") {
  const normalized = normalizeExpense(row || {});
  return {
    ...normalized,
    id: normalized.id || `local_${Math.random().toString(36).slice(2, 9)}`,
    currency: normalized.currency || fallbackCurrency,
    companyCurrency: normalized.companyCurrency || fallbackCurrency,
    category: normalized.category || "Other",
    description: normalized.description || "Expense",
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0a0b0f", color: "#e8eaf0", fontFamily: "'DM Sans', system-ui, sans-serif", display: "grid", gridTemplateColumns: "240px 1fr" }}>
      ...
      <main style={{ padding: 24 }}>
        ...
        {view === "expenses" && <ExpenseList title="My Expenses" rows={myExpenses} company={company} onOpen={(expense) => setModal({ type: "detail", expense: ensureExpense(expense, company?.currency), canAct: false })} />}
        {view === "approvals" && <ExpenseList title="Pending Approvals" rows={pendingApprovals} company={company} onOpen={(expense) => setModal({ type: "detail", expense: ensureExpense(expense, company?.currency), canAct: true })} />}
        {view === "team" && <ExpenseList title="Team Expenses" rows={expenses} company={company} onOpen={(expense) => setModal({ type: "detail", expense: ensureExpense(expense, company?.currency), canAct: false })} />}
      ...
      {modal?.type === "detail" && modal?.expense && (
        <DetailModal
...
function DetailModal({ expense, onClose, onDecision, canAct, company }) {
  ...
  const safeExpense = ensureExpense(expense, company?.currency || "USD");
  return (
    <ModalWrap onClose={onClose}>
      <h3 style={{ marginTop: 0 }}>{safeExpense.description}</h3>
      <div style={{ color: "#6c7a9c", fontSize: 12, marginBottom: 10 }}>{safeExpense.category} · {safeExpense.date}{safeExpense.submittedByName ? ` · ${safeExpense.submittedByName}` : ""}</div>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 8 }}>
        <div style={{ padding: 10, border: "1px solid #1e2130", borderRadius: 10 }}>{fmt(safeExpense.amount, safeExpense.currency)}</div>
        <div style={{ padding: 10, border: "1px solid #1e2130", borderRadius: 10 }}>{fmt(safeExpense.convertedAmount, safeExpense.companyCurrency || company.currency)}</div>
      </div>
      {canAct ? (
        <>
          <Input label="Comment" value={comments} onChange={setComments} />
          <div style={{ display: "flex", gap: 8 }}>
            <button style={{ ...primaryBtn, background: "linear-gradient(135deg,#10b981,#059669)" }} onClick={() => onDecision(safeExpense, "APPROVED", comments)}>Approve</button>
            <button style={{ ...primaryBtn, background: "linear-gradient(135deg,#ef4444,#dc2626)" }} onClick={() => onDecision(safeExpense, "REJECTED", comments)}>Reject</button>
          </div>
        </>
      ) : null}
...
function StatusBadge({ status }) {
  ...
  const palette = {
    approved: { color: "#34d399", bg: "rgba(52,211,153,.12)", border: "rgba(52,211,153,.35)" },
    rejected: { color: "#ef4444", bg: "rgba(239,68,68,.12)", border: "rgba(239,68,68,.35)" },
    pending: { color: "#f59e0b", bg: "rgba(245,158,11,.12)", border: "rgba(245,158,11,.35)" },
  };
